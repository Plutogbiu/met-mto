...
| 本地文件存储 | 开发阶段保存图片、报告文件、附件；上服务器后可切换 MinIO |
...
| 文件存储 | 本地文件存储 | 开发阶段简单直接，上服务器后再切换 MinIO |
| 报告生成 | 后端 HTML 模板生成 PDF | 适合维保报告和巡检回执报告 |
| 地图服务 | 腾讯地图 API | 适合国内定位、地址解析和导航场景 |
| 部署 | 本地运行 / 后续服务器部署 | 开发阶段先不引入 Docker Compose |
...
建议先做模块化单体。后端代码目录采用“顶层分层”，不要按业务模块再嵌套 `controller/service/mapper`。

```text
mto-server/src/main/java/com/met/mto
├── common      通用返回、异常、工具
├── config      配置类
├── controller  所有 Controller
├── service     所有 Service
├── mapper      所有 MyBatis Mapper
├── entity      数据库实体
├── dto         请求和响应 DTO
└── MtoServerApplication.java
```
...
1. 业务中文可以继续称为“煤矿”。
2. 代码、表名和接口路径不使用 `mine`。
3. 与煤矿客户现场相关的对象统一使用 `customerSite` / `customer_site` / `customer-sites`。
4. 例如：`CustomerSiteController`、`customer_site`、`/api/admin/customer-sites`。
...
开发阶段先使用本地文件目录作为业务存储，上服务器时再切换 MinIO。

文件路径按日期、业务和分类组织：

```text
uploads/ticket/2026/06/03/{ticketId}/before/xxx.jpg
uploads/ticket/2026/06/03/{ticketId}/after/xxx.jpg
uploads/ticket/2026/06/03/{ticketId}/scene/xxx.jpg
uploads/inspection/2026/06/03/{ticketId}/xxx.jpg
uploads/report/2026/06/03/{ticketId}/maintenance.pdf
```

数据库先保存文件相对路径、原始文件名、文件类型、大小、上传人、关联工单。后续切换 MinIO 时，再扩展 bucket 和 object key。
...
### 6.2 客户现场表 customer_site 初版定位字段
...
| /api/admin/customer-sites | GET/POST | 客户现场列表/新增 |
| /api/admin/customer-sites/{id} | GET/PUT | 客户现场详情/编辑 |
| /api/admin/customer-sites/{id}/location | PUT | 更新客户现场坐标 |
| /api/admin/customer-sites/{id}/inspection-items | GET/POST | 巡检清单 |
...
如果后续希望部署更规整，可以使用 Docker Compose：
...
开发阶段不强制使用 Redis、消息队列和 MinIO。

### 10.3 2G 轻量服务器建议

2G 内存服务器可以做内部试运行，但不建议作为长期稳定生产配置。

原因是本系统如果同机部署 Nginx、后端服务、MySQL，内存已经需要控制；如果后续再加 MinIO，内存会更紧。若预算允许，服务器建议至少升级到 4G 内存；如果只能使用 2G，则必须控制资源并接受偶发内存压力风险。
...
5. PDF 报告生成不要并发批量执行。
6. MySQL、uploads 文件和报告文件都要做定时备份。
...
uploads/ticket/2026/06/03/{ticketId}/before/xxx.jpg
uploads/ticket/2026/06/03/{ticketId}/after/xxx.jpg
uploads/report/2026/06/03/{ticketId}/xxx.pdf
```
...
| --- | --- |
| 图片水印 | 上传接口统一处理图片 |
| MinIO 切换 | 文件服务封装统一接口，业务不直接依赖本地路径 |
| 工单审核 | 工单状态保留扩展字段 |
| 电子签名 | 报告模块预留签名图片 |
| 远程工单 | 工单类型可扩展 |
